<template>
  <Menu as="div" class="relative inline-block text-left">
    <div>
      <MenuButton
          class="inline-flex w-full justify-center gap-x-1.5 rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
        {{ selectedChoice }}
        <ChevronDownIcon class="-mr-1 h-5 w-5 text-gray-400" aria-hidden="true"/>
      </MenuButton>
    </div>

    <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
                enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75"
                leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
      <MenuItems
          class="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
        <div class="py-1">
          <MenuItems v-if="this.choices !== undefined" v-slot="Test" v-for="item in this.choices">
            <a href="#" @click="madeChoice(item)" :class="[Test ? 'bg-gray-100 text-gray-900 hover:bg-gray-400' : 'text-gray-700', 'block px-4 py-2 text-sm']">{{ item }}</a>
          </MenuItems>
          <MenuItems v-else v-slot="Test" v-for="item in this.mapChoices">
            <a href="#" @click="madeMapChoice(item)" :class="[Test ? 'bg-gray-100 text-gray-900 hover:bg-gray-400' : 'text-gray-700', 'block px-4 py-2 text-sm']">{{ item.name }}</a>
          </MenuItems>
        </div>
      </MenuItems>
    </transition>
  </Menu>
</template>

<script>
import {Menu, MenuButton, MenuItems} from "@headlessui/vue";
import {ChevronDownIcon} from "@heroicons/vue/20/solid";

export default {
  props: {
    choices: {
      type: Array,
      required: false,
    },
    mapChoices: {
      type: Array,
      required: false,
    },
  },
  components: {
    Menu,
    MenuButton,
    ChevronDownIcon,
    MenuItems,
  },
  data() {
    return {
      selectedChoice: this.choices !== undefined ? this.choices[0] : this.mapChoices[0].name,
    };
  },
  methods: {
    madeChoice(name) {
      console.log(name)
      this.selectedChoice = name
    },
    madeMapChoice(item) {
      console.log(item.name)
      console.log(item.id)
      this.selectedChoice = item.name
      this.$emit('changeMapChoice', item)
    },
  },
  created() {

  }
}

</script>

<script setup>


</script>